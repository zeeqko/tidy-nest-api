...
func (c *InventoryController) List(w http.ResponseWriter, r *http.Request) {
	items, err := c.service.List(r.Context(), CurrentUser(r.Context()).ID)
	if err != nil {
...
func (c *InventoryController) Get(w http.ResponseWriter, r *http.Request) {
	...
	item, err := c.service.Get(r.Context(), CurrentUser(r.Context()).ID, id)
	if err != nil {
...
func (c *InventoryController) Create(w http.ResponseWriter, r *http.Request) {
	...
	created, err := c.service.Create(r.Context(), CurrentUser(r.Context()).ID, item)
	if err != nil {
		writeError(w, http.StatusInternalServerError, err)
		return
	}
...
func (c *InventoryController) Update(w http.ResponseWriter, r *http.Request) {
	...
	updated, err := c.service.Update(r.Context(), CurrentUser(r.Context()).ID, id, item)
	if err != nil {
		writeServiceError(w, err)
		return
	}
